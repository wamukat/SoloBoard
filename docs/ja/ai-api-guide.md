...
```bash
tmpdir=$(mktemp -d)
curl -L https://github.com/wamukat/kanbalone/archive/refs/tags/v0.9.22.tar.gz \
  | tar -xz -C "$tmpdir" kanbalone-0.9.22/skills/kanbalone-api
mkdir -p "${CODEX_HOME:-$HOME/.codex}/skills"
cp -R "$tmpdir"/kanbalone-0.9.22/skills/kanbalone-api "${CODEX_HOME:-$HOME/.codex}/skills/"
rm -rf "$tmpdir"
```
...
### 9. `/api/meta` で remote provider の利用可否を返す
...
### 10. `/api/remote-diagnostics` で remote credential を診断する
