...
SoloBoard の UI は、Plane.so や Linear のような精度の高いプロダクト UI を参考にしながら、ローカルファーストの軽さと個人開発者向けの単純さを優先します。
...
SoloBoard は派手な SaaS ダッシュボードではなく、毎日開き続けても疲れない作業面です。装飾は控えめにし、境界・余白・タイポグラフィで品質を出します。
...
SoloBoard ではカードとボタンの radius は 8px 以下にします。丸みは控えめにし、Plane/Linear 系の精密さを保ちます。
