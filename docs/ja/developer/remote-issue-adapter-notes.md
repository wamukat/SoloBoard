...
`pushing` は remote への重複投稿を避けるための短時間の中間状態です。異常終了などで残った場合は、一定時間後に再 push で復旧できる前提にします。
...
- UI で `Push to remote` / `Pushing` / `Pushed` / `Retry` を出す
...
- `provider:*` fallback は Redmine など path-based/self-hosted provider 向けに限定する。GitHub/GitLab は任意 origin への token 送信を避けるため wildcard を使わず、origin credential を明示する
...
- credential key の `instanceUrl` は provider ごとに正規化する。GitHub/GitLab は origin 単位に畳み、Redmine は path-based deployment のため path を保持する。
