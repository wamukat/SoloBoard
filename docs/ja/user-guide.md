...
現在の UI は、チケットカード、レーン、フィルタ、ダイアログを同じデザインルールで揃えています。淡い境界線、控えめな影、必要な時だけ出る操作ボタンで、作業内容に集中しやすくしています。
...
レーン名の変更や削除は、各レーン右上のアクションボタンから開けます。レーン名の変更はダイアログではなく、レーン見出しをその場で編集します。普段使わない操作は少し奥に置いてあるので、ボードを見る時のノイズが少なくなります。
...
必要な時は、タグフィルタの右にある `Status >` を開き、`Archived` を追加すると、アーカイブ済みチケットも表示されます。`Resolved` も同じメニューから追加できます。条件を解除したい時は、フィルタ内の `x` アイコンを押します。
...
その代わり、OS やブラウザの外観設定がダークモードの場合、自動的にダークテーマで表示されます。明るい環境ではライトテーマ、暗い環境ではダークテーマというように、普段使っている端末設定に従います。
