...
docker run --rm \
  ...
  ghcr.io/wamukat/kanbalone:v0.9.16
```
...
```bash
tmpdir=$(mktemp -d)
curl -L https://github.com/wamukat/kanbalone/archive/refs/tags/v0.9.16.tar.gz \
  | tar -xz -C "$tmpdir" kanbalone-0.9.16/skills/kanbalone-api
mkdir -p "${CODEX_HOME:-$HOME/.codex}/skills"
cp -R "$tmpdir"/kanbalone-0.9.16/skills/kanbalone-api "${CODEX_HOME:-$HOME/.codex}/skills/"
rm -rf "$tmpdir"
```
