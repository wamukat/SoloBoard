...
type BoardRoutesSchemas = {
  ...
  ticketRemoteImportPreviewResponseSchema: unknown;
...
export function registerBoardRoutes(app: FastifyInstance, ctx: RegisterBoardRoutesContext): void {
  ...
  app.post("/api/boards/:boardId/remote-import/preview", {
    ...
    try {
      ...
      return serializeRemoteImportPreview(snapshot);
    } catch (error) {
      return reply.code(400).send({ error: sanitizeRemoteIssueLookupError(error, "remote import preview failed") });
    }
  });

  function serializeRemoteImportPreview(snapshot: RemoteIssueSnapshot): unknown {
    const existingTicketId = db.findTicketIdByRemoteIdentity({
      provider: snapshot.provider,
      instanceUrl: snapshot.instanceUrl,
      resourceType: snapshot.resourceType,
